...
class Snake(Entity):  
    ...
    def mudar_direcao(self, tecla): 
        # Evita a mudança de direção de 180 graus
        if tecla == K_LEFT and self.direcao == K_RIGHT:
            return
        if tecla == K_RIGHT and self.direcao == K_LEFT:
            return
        if tecla == K_UP and self.direcao == K_DOWN:
            return
        if tecla == K_DOWN and self.direcao == K_UP:
            return
